...
class SegmentationNetworkWithClassificationHead(nn.Module):
    def __init__(self, seg_network: nn.Module, features_per_stage: List[int],
        ...
        self.conv_block = nn.Sequential(
            nn.Conv3d(features_per_stage[-1], features_per_stage[-1]*2, kernel_size=3, padding=1),
            nn.BatchNorm3d(features_per_stage[-1]*2),
            nn.ReLU(inplace=True),
            nn.Conv3d(features_per_stage[-1]*2, features_per_stage[-1]*2, kernel_size=3, padding=1),
            nn.BatchNorm3d(features_per_stage[-1]*2),
            nn.ReLU(inplace=True)
        )
        ...
        # Classification layers
        self.classifier = nn.Sequential(
            nn.Linear(features_per_stage[-1]*2, features_per_stage[-1]),
            nn.LeakyReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(features_per_stage[-1], num_classes)
        )
...
class GRAPE(nn.Module):
    def __init__(self, seg_network: nn.Module, features_per_stage: List[int],
        ...
        # Classification layers
        self.classifier = nn.Sequential(
            nn.Linear(features_per_stage[-3], features_per_stage[-3]),
            nn.LeakyReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(features_per_stage[-3], num_classes)
        )
        self.classifier2 = nn.Sequential(
            nn.Linear(features_per_stage[-2], features_per_stage[-2]),
            nn.LeakyReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(features_per_stage[-2], num_classes)
        )
        self.classifier3 = nn.Sequential(
            nn.Linear(features_per_stage[-1], features_per_stage[-1]),
            nn.LeakyReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(features_per_stage[-1], num_classes)
        )
...
class nnUNetCLSTrainer(nnUNetTrainer):
    def initialize(self):
        if not self.was_initialized:
            ...
            self.seg_weight = 1.0
            self.cls_weight = 0.0
            self.dynamic_loss_weight_updater = DynamicLossWeightUpdater(window_size=5, momentum=0.9)

            wandb.init(
                project="nnUNet_cls_cvpr26",
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        l = self.seg_weight*seg_loss + self.cls_weight*cls_loss
...
class BaselineCLS(nnUNetCLSTrainer):
    ...
    def on_epoch_end(self):
        ...
        # handle 'best' checkpointing. ema_fg_dice is computed by the logger and can be accessed like this
        eval_metric = self.logger.my_fantastic_logging['ema_fg_dice'][-1] + \
            self.logger.my_fantastic_logging['val_classification_accuracy'][-1]
        if self._best_ema is None or eval_metric > self._best_ema:
            self._best_ema = eval_metric
            self.print_to_log_file(f"Yayy! New best EMA pseudo Dice: {np.round(self._best_ema, decimals=4)}")
            self.save_checkpoint(join(self.output_folder, 'checkpoint_best.pth'))
...
class PANORAMA(nnUNetTrainer):
    def on_epoch_end(self):
        ...
        # handle 'best' checkpointing. ema_fg_dice is computed by the logger and can be accessed like this
        eval_metric = self.logger.my_fantastic_logging['ema_fg_dice'][-1] + \
            self.logger.my_fantastic_logging['dice_per_class_or_region'][-1][0]
        if self._best_ema is None or eval_metric > self._best_ema:
            self._best_ema = eval_metric
            self.print_to_log_file(f"Yayy! New best score: {np.round(self._best_ema, decimals=4)}, Lesion dice: {np.round(self.logger.my_fantastic_logging['dice_per_class_or_region'][-1][0], decimals=4)}")
            self.save_checkpoint(join(self.output_folder, 'checkpoint_best.pth'))
...
class PanoramaPretrained(nnUNetCLSTrainer):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
    ...
    def on_epoch_end(self):
        ...
        # handle 'best' checkpointing. ema_fg_dice is computed by the logger and can be accessed like this
        eval_metric = self.logger.my_fantastic_logging['dice_per_class_or_region'][-1][0] + \
            self.logger.my_fantastic_logging['val_auc'][-1]
        if self._best_ema is None or eval_metric > self._best_ema:
            self._best_ema = eval_metric
            self.print_to_log_file(f"Yayy! New best EMA pseudo Dice: {np.round(self._best_ema, decimals=4)}")
...
class HecktorPretrained(nnUNetCLSTrainer):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class LunaCLS(BaselineCLS):
    def on_epoch_end(self):
        ...
        if self._best_ema is None or eval_metric > self._best_ema:
            self._best_ema = eval_metric
            self.print_to_log_file(f"Yayy! New best EMA score: {np.round(self._best_ema, decimals=4)}")
            self.save_checkpoint(join(self.output_folder, 'checkpoint_best.pth'))
...
class MamamiaPretrained(nnUNetCLSTrainer):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class PDCADPretrained(nnUNetCLSTrainer):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class PanoramaMTL(nnUNetCLSTrainerMTL):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class HecktorMTL(nnUNetCLSTrainerMTL):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class MamamiaMTL(nnUNetCLSTrainerMTL):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class LunaMTL(nnUNetCLSTrainerMTL):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
...
class PDCADPretrainedMTL(nnUNetCLSTrainerMTL):
    def initialize(self):
        if not self.was_initialized:
            ## DDP batch size and oversampling can differ between workers and needs adaptation
            # we need to change the batch size in DDP because we don't use any of those distributed samplers
